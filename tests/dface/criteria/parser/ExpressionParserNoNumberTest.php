...
class ExpressionParserNoNumberTest extends TestCase
{
	...
	private function assertExpressionMatchNode($exp, Node $node) : void
	{
		$parsed = $this->parser->parse($exp);
		self::assertEquals($node, $parsed, "Expression $exp must match node");
	}

	private function assertExpressionNotMatchNode($exp, Node $node) : void
	{
		$parsed = $this->parser->parse($exp);
	...
	private function doComparisonTest(Node $x1_node, $operator, $bad_x1_node, $bad_operator) : void
	{
	...
	{
		$x = new Equals(new Reference('x'), new StringConstant(1));
		$y = new Equals(new Reference('y'), new StringConstant(2));
		$z = new Equals(new Reference('z'), new StringConstant(3));
	...
	{
		$set = [new StringConstant(1), new StringConstant(2), new StringConstant(3)];
		$bad_set = [new StringConstant(1), new StringConstant(2)];
